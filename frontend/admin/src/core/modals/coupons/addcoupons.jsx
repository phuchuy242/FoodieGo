...
const AddCoupons = () => {
    ...
    const [endDate, setEndDate] = useState(new Date(new Date().setDate(new Date().getDate() + 30))); // default 30 days
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.post(`${API_BASE}/api/v1/admin/vouchers/`, payload, { headers });

            Swal.fire({ title: 'Thành công', text: 'Đã tạo mã giảm giá mới!', icon: 'success' });
                                        ...
                                        <div className="row">
                                            ...
                                            <div className="input-blocks m-0 mt-3">
                                                <div className="status-toggle modal-status d-flex justify-content-between align-items-center">
                                                    ...
                                                    <input
                                                        type="checkbox"
                                                        id="add_voucher_status"
                                                        className="check"
                                                        checked={isActive}
                                                        onChange={(e) => setIsActive(e.target.checked)}
                                                    />
                                                    <label htmlFor="add_voucher_status" className="checktoggle" />
                                                </div>
                                            </div>
                                        </div>
                                        <div className="modal-footer-btn mt-4">
                                            <button type="button" className="btn btn-cancel me-2" data-bs-dismiss="modal">Hủy</button>
                                            <button type="submit" className="btn btn-submit" disabled={loading}>{loading ? 'Đang lưu...' : 'Thêm mới'}</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
